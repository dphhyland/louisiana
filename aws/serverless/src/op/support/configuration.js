      ...
      mTLS: {
        ...
        certificateSubjectMatches(ctx, property, expected) {
          if (property !== 'tls_client_auth_subject_dn') {
            console.log(`${property} is not supported by this deployment`);
            throw new Error(`${property} is not supported by this deployment`);
          }
          const subject = ctx.get('X-Client-DN');
          if (subject === expected) {
            return true;
          }
        ...
        getCertificate(ctx) {
          return ctx.get('X-Client-Cert');
        },
        certificateAuthorized(ctx) {
          return (
              ctx.get('X-Client-Cert')
          );
        },
