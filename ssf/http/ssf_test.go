...
func TestMain(m *testing.M) {
	...
	testCollection = setupTestCollection(nil)
...
func setupTestCollection(t *testing.T) *mongo.Collection {
	...
	testCollection := testClient.Database("test_signals_db").Collection("test_streams")

	// Clear the testCollection before each test
	_, err = testCollection.DeleteMany(testCtx, bson.M{})
	assert.NoError(t, err, "Failed to clear test testCollection")
...
func TestProduceEventHandler(t *testing.T) {
	...
	r.Post("/produce-event", func(w http.ResponseWriter, r *http.Request) {
		ProduceEventHandler(w, r, mockProducer)
	})
...
func TestAddSubjectToStreamSection5(t *testing.T) {
	...
	// Wait briefly to allow the database to process the insertion
	...
	// Log the current subjects for debugging
	fmt.Printf("Subjects in the stream: %+v\n", updatedStreamConfig.Subjects)

	// Check that the subjects array contains the expected data
	assert.Len(t, updatedStreamConfig.Subjects, 1, "Expected exactly one subject in the stream")
...
// 	collection = client.Database("test_signals_db").Collection("test_streams")
// 	// Clear the collection before the test
// 	_, err = collection.DeleteMany(ctx, bson.M{})
// 	if err != nil {
// 		t.Fatalf("Failed to clear test collection: %v", err)
// 	}
// 	streamID := "f67e39a0a4d34d56b3aa1bc4cff0069x"
...
// 	// Create request
// 	req := httptest.NewRequest(http.MethodPut, "/stream-config/"+streamID, bytes.NewBuffer([]byte(tokenString)))
// 	req.Header.Set("Content-Type", "application/jwt")
// 	rec := httptest.NewRecorder()
...
// 	var updatedStreamConfig StreamConfig
// 	err = collection.FindOne(ctx, bson.M{"stream_id": streamID}).Decode(&updatedStreamConfig)
// 	if err != nil {
// 		t.Fatalf("Failed to retrieve updated stream configuration: %v", err)
// 	}

// 	if updatedStreamConfig.Status != "paused" {
// 		t.Errorf("Expected stream status to be 'paused', got '%s'", updatedStreamConfig.Status)
// 	}

// 	if updatedStreamConfig.Reason == nil || *updatedStreamConfig.Reason != "Maintenance" {
// 		t.Errorf("Expected stream reason to be 'Maintenance', got '%v'", updatedStreamConfig.Reason)
// 	}
// }
